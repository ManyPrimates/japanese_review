...
```{r}
# TODO check whether these should be excluded
# if not, remove the comment in the "to_exclude" column
# if yes, delete that line
studies %>% 
    filter(str_detect(to_exclude, "exclude")) %>% 
    mutate(authors = str_sub(authors, 1, 15), title = str_sub(title, 1, 35)) %>% 
    select(row, authors, title, year)

studies <- studies %>% filter(is.na(to_exclude)) %>% select(-to_exclude)
```
...
sort(unique(studies$title)) %>% str_sub(1, 25)
```
