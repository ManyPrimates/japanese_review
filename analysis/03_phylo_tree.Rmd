...
pcol <- ggplot(tibble(cols = cols, x = 1:4), aes(x, y = 1, col = cols)) +
  ...
  scale_color_identity("Clade", guide = "legend", breaks = cols[4:1], 
                       labels = c("Homonoidea", "Catarrhini", "Platyrrhini", 
                                  "Tarsiformes & Strepsirrhini")) +
  theme_cowplot()
...
q <- ggtree(tree5, aes(col = group)) +
  ...
  geom_text(aes(label = Nsites), x = 128, hjust = 1, size = 3) +
  geom_text(aes(label = Nstudies), x = 135, hjust = 1, size = 3) +
  # tweak scales
