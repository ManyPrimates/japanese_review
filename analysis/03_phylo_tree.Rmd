...
# for vertical line in ridge plot
v <- tibble(.panel = c("Tree", "ٴSample sizes"),
            reference = c(NA, median(data3$num))) # grand median
```
...
q <- ggtree(tree5, aes(col = group)) +
  ...
  # add reference line (this will show up on right panel of facet_plot only)
  geom_vline(data = v, aes(xintercept = reference), lty = 3, col = "grey") +
  # display timescale at the bottom
  theme_tree2()
```
